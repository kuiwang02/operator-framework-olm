...
1. **LEVEL0**: Add `[Level0]` in the case title, not in the `g.Label()`.
...
10. **DEPRECATED**: Deprecated, corresponding cases deprecated. Don't add this kind of case to here. But, if you already add it, please use `IgnoreObsoleteTests` instead of `DEPRECATED` strings.
